...
class _WorkspaceScreenState extends State<WorkspaceScreen> {
  ...
  Future<void> _syncCanonical() async {
    if (_root == null) return;
    setState(() => _message = 'Validating and syncing Git canonical data…');
    try {
      final report = await CanonicalSyncService(Workspace(_root!))
          .syncCanonical(
            message: 'worklog: sync canonical data',
            verifier: const CanonicalSecretVerifier(),
          );
      _refresh();
      setState(
        () => _message =
            'Git sync complete · committed=${report.committed} · '
            'rebased=${report.rebased} · pushed=${report.pushed}',
      );
    } catch (error) {
      setState(() => _message = 'Git sync blocked · $error');
    }
  }
  ...
  Future<void> _generateMeta() async {
    ...
    try {
      ...
      adapters = registry
          .list()
          .where(
            (descriptor) => descriptor.capabilities.contains('generate_meta'),
          )
          .where((descriptor) {
            try {
              registry.validate(
                descriptor,
                requiredCapability: 'generate_meta',
              );
              return true;
            } on Object {
              return false;
      ...
      setState(() => _message = 'Runtime registry blocked · $error');
      return;
    }
    if (adapters.isEmpty) {
      setState(
        () => _message =
            'No verified runtime adapter with generate_meta is installed.',
      );
      return;
    }
    var selectedId = adapters.first.id;
    final accepted = await showDialog<bool>(
      context: context,
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          title: const Text('Generate Meta Prompt'),
          content: DropdownButtonFormField<String>(
            initialValue: selectedId,
            decoration: const InputDecoration(labelText: 'Verified adapter'),
            items: [
              for (final adapter in adapters)
                DropdownMenuItem(value: adapter.id, child: Text(adapter.id)),
            ],
            onChanged: (value) {
    ...
    if (accepted != true) return;
    setState(() => _message = 'Generating Meta Prompt…');
    try {
      final result = await MetaPromptService(
        workspace,
      ).generate(taskId: task.id, adapterId: selectedId);
      _refresh();
      setState(() {
        _selected = result.task;
        _message = 'Meta generated by ${result.adapterId} · approval required.';
      });
    } catch (error) {
      setState(() => _message = 'Meta generation blocked · $error');
    }
  }
  ...
  List<String> _runSummaries(WorkTask task) {
    final repository = CanonicalRepository(Workspace(_root!));
    final runs = repository
        .list(EntityKind.run)
        .where((run) => run.data['task_id'] == task.id)
        .toList()
        .reversed;
    return [
      for (final run in runs)
        _runSummary(
          ...
          repository
              ...
              .where((item) => item.data['run_id'] == run.id)
              .toList()
            ..sort(
          ...
          repository
              ...
              .where(
                (item) =>
                    item.data['run_id'] == run.id &&
                    item.data['event_type'] == 'orchestration_completed',
              )
              .lastOrNull,
          repository
  ...
  static String _runSummary(
    CanonicalEntity run,
    List<CanonicalEntity> invocations,
    CanonicalEntity? completion,
    CanonicalEntity? claim,
  ) {
    final pipeline = invocations.isEmpty
        ? 'pipeline pending'
        : invocations
              .map(
                (item) =>
                    '${item.data['skill_id']}'
                    '${(item.data['round'] as int? ?? 0) > 0 ? " r${item.data['round']}" : ""}',
              )
              .join(' → ');
    final score = completion?.data['reviewer_score'];
    final heartbeat = claim?.data['heartbeat_at'];
    return '${run.id} · ${run.data['status']} · $pipeline'
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            key: const Key('git-sync-now'),
            tooltip: 'Sync Git canonical data',
            onPressed: _root == null ? null : _syncCanonical,
                ...
                Expanded(
                  child: _selected == null
                      ...
                      : _TaskDetail(
                          ...
                          runSummaries: _runSummaries(_selected!),
...
class _TaskDetail extends StatelessWidget {
  ...
  final List<String> runSummaries;
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            ...
            TextButton.icon(
              ...
              label: const Text('Generate Meta'),
            ),
            TextButton.icon(
        ...
        const SizedBox(height: 28),
        Text('Run activity', style: Theme.of(context).textTheme.titleMedium),
        if (runSummaries.isEmpty)
          ...
          for (final summary in runSummaries)
            ListTile(
